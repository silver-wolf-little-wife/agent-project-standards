...
将 `SKILL.md`（英文版）或 `SKILL.zh.md`（中文版）作为 skill 或 prompt 上下文加载。Agent 会自动：
...
将 `SKILL.md` 或 `SKILL.zh.md` 作为参考指南，用于：
...
Load `SKILL.md` (English) or `SKILL.zh.md` (Chinese) as a skill/prompt context. The agent will automatically:
...
Read `SKILL.md` or `SKILL.zh.md` as a reference guide for:
